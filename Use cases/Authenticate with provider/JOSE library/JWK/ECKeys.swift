...
public struct ECPublicKey: JWK {
    ...
    public init(crv: ECCurveType, x: String, y: String, additionalParameters parameters: [String: AnyCodable] = [:]) {
        self.keyType = .EC
        self.crv = crv
        self.x = x
        self.y = y
        ...
        self.parameters = (parameters).merging(
                [
                    JWKParameter.keyType.rawValue: AnyCodable(self.keyType.rawValue),
                    ECParameter.curve.rawValue: AnyCodable(self.crv.rawValue),
                    ECParameter.x.rawValue: AnyCodable(self.x),
                    ECParameter.y.rawValue: AnyCodable(self.y)
                ],
    ...
    /// - Throws: A `JOSESwiftError` indicating any errors.
    public init(publicKey: ExpressibleAsECPublicKeyComponents, additionalParameters parameters: [String: AnyCodable] = [:]) throws {
...
public struct ECPrivateKey: JWK {
    ...
    ///                 as specified in [RFC-7518, Section 2](https://tools.ietf.org/html/rfc7518#section-2).
    ///   - parameters: Additional JWK parameters.
    public init(crv: String, x: String, y: String, privateKey: String, additionalParameters parameters: [String: AnyCodable] = [:]) throws {
        self.keyType = .EC
        ...
        self.parameters = (parameters).merging(
                [
                    ...
                    ECParameter.x.rawValue: AnyCodable(self.x),
                    ECParameter.y.rawValue: AnyCodable(self.y),
                    ECParameter.privateKey.rawValue: AnyCodable(self.privateKey)
                ],
                uniquingKeysWith: { (_, new) in new }
            )
    }
    ...
    ///   - parameters: Any additional parameters to be contained in the JWK.
    /// - Throws: A `JOSESwiftError` indicating any errors.
    public init(privateKey: ExpressibleAsECPrivateKeyComponents, additionalParameters parameters: [String: AnyCodable] = [:]) throws {
