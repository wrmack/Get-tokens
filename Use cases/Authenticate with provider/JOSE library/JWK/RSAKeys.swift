...
public struct RSAPublicKey: JWK {
    ...
    public init(modulus: String, exponent: String, additionalParameters parameters: [String: AnyCodable] = [:]) {
        self.keyType = .RSA
        ...
        self.parameters = (parameters).merging(
            [
                ...
                RSAParameter.modulus.rawValue: AnyCodable(self.modulus),
                RSAParameter.exponent.rawValue: AnyCodable(self.exponent)
            ],
    ...
    /// - Throws: A `JOSESwiftError` indicating any errors.
    public init(publicKey: ExpressibleAsRSAPublicKeyComponents, additionalParameters parameters: [String: AnyCodable] = [:]) throws {
...
public struct RSAPrivateKey: JWK {
    ...
    ///               as specified in [RFC-7518, Section 2](https://tools.ietf.org/html/rfc7518#section-2).
    ///   - parameters: Additional JWK parameters.
    public init(modulus: String, exponent: String, privateExponent: String, additionalParameters parameters: [String: AnyCodable] = [:]) {
        self.keyType = .RSA
        self.modulus = modulus
        self.exponent = exponent
        ...
        self.parameters = (parameters).merging(
            [
                ...
                RSAParameter.modulus.rawValue: AnyCodable(self.modulus),
                RSAParameter.exponent.rawValue: AnyCodable(self.exponent),
                RSAParameter.privateExponent.rawValue: AnyCodable(self.privateExponent)
            ],
            uniquingKeysWith: { (_, new) in new }
            )
    }
    ...
    ///   - parameters: Any additional parameters to be contained in the JWK.
    /// - Throws: A `JOSESwiftError` indicating any errors.
    public init(privateKey: ExpressibleAsRSAPrivateKeyComponents, additionalParameters parameters: [String: AnyCodable] = [:]) throws {
