...
public struct SymmetricKey: JWK {
    ...
    public init(key: Data, additionalParameters parameters: [String: AnyCodable] = [:]) {
        self.keyType = .OCT
        ...
        self.parameters = (parameters).merging(
            [
                ...
                SymmetricKeyParameter.key.rawValue: AnyCodable(self.key)
            ],
    ...
    /// - Throws: A `JOSESwiftError` indicating any errors.
    public init(key: ExpressibleAsSymmetricKeyComponents, additionalParameters parameters: [String: AnyCodable] = [:]) throws {
