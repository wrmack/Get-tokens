...
class ErrorUtilities {
    ...
    static func OAuthError(OAuthErrorDomain: String, OAuthResponse errorResponse:Dictionary<String, Any>?, underlyingError: NSError?) -> NSError {
        ...
        if errorResponse![OIDOAuthErrorFieldErrorDescription] is String {
            oauthErrorMessage = (errorResponse![OIDOAuthErrorFieldErrorDescription] as! String).description
        }
        
        var oauthErrorURI: String?
        
        if errorResponse![OIDOAuthErrorFieldErrorURI] is String {
            oauthErrorURI = (errorResponse![OIDOAuthErrorFieldErrorURI] as! String)
        }
        else {
            oauthErrorURI = errorResponse![OIDOAuthErrorFieldErrorURI].debugDescription
        }
        ...
        if description.count == 0 {
            description.append("OAuth error: \(oauthErrorCodeString) - https://tools.ietf.org/html/rfc6749#section-5.2")
        }
    ...
    static func raiseException(name: String) {
        ErrorUtilities.raiseException(name: name, message: name)
    }
    
    
    static func raiseException(name: String, message: String?) {
        // TODO:
        //  NSException.raise(NSExceptionName(rawValue: name), format: "%@", arguments:  getVaList())
    }
