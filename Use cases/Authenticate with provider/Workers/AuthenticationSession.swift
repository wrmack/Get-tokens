...
class AuthenticationSession: NSObject, URLSessionDelegate  {
    var authorizationRequest: AuthorizationRequest?
    var presentingViewController: UIViewController?
    var externalUserAgentFlowInProgress = false
    var webAuthenticationVC: ASWebAuthenticationSession?
    var pendingauthorizationFlowCallback: ((AuthState?, Error?)->Void)?
    ...
    func fetchAuthState(authorizationRequest: AuthorizationRequest?, presentingViewController: UIViewController?, callback: @escaping (AuthState?, Error?) -> Void) -> AuthenticationSession {
        ...
        let authorizationFlowStarted = presentAuthenticationViewController(request: authorizationRequest)  { authorizationResponse, authorizationError in
        ...
        if !authorizationFlowStarted {
            ...
            didFinish(with: nil, error:safariError)
        }
        return self
    }
     ...
     */
    func presentAuthenticationViewController(request: AuthorizationRequest?, callback: @escaping (AuthorizationResponse?, Error?) -> Void) -> Bool {
 ...
 //           cleanUp()
    ...
    func resumeExternalUserAgentFlow(with URL: URL?, callback:  ((AuthorizationResponse?, Error?) -> Void)?) -> Bool {
